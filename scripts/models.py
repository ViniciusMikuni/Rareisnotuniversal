...
def _AE(NFEAT,LAYERSIZE,ENCODESIZE):
    ...
    for il in range(len(LAYERSIZE)):
        layer = Dense(LAYERSIZE[len(LAYERSIZE)-il-1], activation='relu', use_bias=False)(layer)
    #layer = Dropout(0.25)(layer)
    outputs = Dense(NFEAT, activation='linear', use_bias=False)(layer)

    return inputs,outputs

def AE(reference,val_data,
       checkpoint_name,
       LAYERSIZE=[30,20,10],ENCODESIZE=1,
    ...
    inputs,outputs = _AE(reference.shape[-1],LAYERSIZE,ENCODESIZE)
...
def FFJORD(reference,val_data,
           checkpoint_name,
           LAYERSIZE=[30,20,10],STACKSIZE=1,
           lr=1e-3,max_epoch=500):
    '''
    Inputs:
    reference: Background events used for training
    ...
    for _ in range(STACKSIZE):
        mlp_model = flow.MLP_ODE(LAYERSIZE, num_output=reference.shape[-1])
        stacked_mlps.append(mlp_model)

    #Create the model

    batch_size = 10000
    model = flow.FFJORD(stacked_mlps,batch_size=batch_size,num_output=reference.shape[-1])
    ...
    val_s,val_b=val_data
    prob_b = -model.prob(val_b)
    prob_s = -model.prob(val_s)
...
def CWoLa(data,reference,val_data,
          checkpoint_name,
          LAYERSIZE=[30,20,10],
          lr=1e-3,max_epoch=500):
    '''
    ...
    inputs,outputs = _classifier(data.shape[-1],LAYERSIZE)
    model = Model(inputs, outputs)
    ...
    pred_s = pred_s/(1-pred_s)
    pred_b = model.predict(val_b,batch_size=1000)
    pred_b = pred_b/(1-pred_b)
